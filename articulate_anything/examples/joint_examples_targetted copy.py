...
def partnet_100248(input_dir, links, joint_id="joint_0"):
    ...
    # ====================JOINT PREDICTION====================
    factor = 0.85  # not fully all the way up
    # to where the handle is at lower and upper limit of the joint
    lower_point = [
        0,
        0,
        0,
...
def partnet_47645(input_dir, links, joint_id="joint_0"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            # axis-motion relationship:
            # We want the lid to open from down to up (lifted up). In our convention, up is positive and
            # down is negative so down to up is negative
            -1,
            0,
...
def partnet_102658(input_dir, links, joint_id="joint_0"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            # axis-motion relationship:
            # We want the toilet lid to open from down to up (lifted up). In our convention, up is positive and
            # down is negative so down to up is negative
            -1,
            0,
...
def partnet_103015(input_dir, links, joint_id="joint_2"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            # axis-motion relationship:
            # We want the window to open from down to up (lifted up). In our convention, up is positive and
            # down is negative so down to up is negative
            -1,
            0,
...
def partnet_41003(input_dir, links, joint_id="joint_1"):
    ...
    # ====================JOINT PREDICTION====================
    # -- Groundtruth video analysis --
    # in the groundtruth video, the top-right door opens up while its back-RIGHT part of the door is still
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            0,
            # motion-axis relationship:
            # We want to door, which is on the right, to open outward (i.e., from left to right).
...
def partnet_41003(input_dir, links, joint_id="joint_3"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            ...
            0,
            # axis-motion relationship:
            # We want the door, which is on the left, to open outward (i.e., from right to left)
            # so set the axis to negative
            -1,
        ],  # The cabinet door swings open and closed around the vertical axis,
...
def partnet_103619(input_dir, links, joint_id="joint_0"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            # axis-motion relationship:
            # the handle is squeezed in i.e., moving from up to down so set the axis to positive
            1,
            0,
...
def partnet_12565(input_dir, links, joint_id="joint_1"):
    ...
    pred_robot.make_revolute_joint(
        ...
        global_axis=[
            0,
            # axis-motion relationship:
            # we want the dishwasher door to open outward
            1,
            0,
